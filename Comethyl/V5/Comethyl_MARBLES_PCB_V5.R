...
#added insertion near NHIP data 
#no FDR correction for ME-trait correlations (adjMethod = "none") . Significant if unadjusted p<0.05

# Setup ###
#ssh jmouat@epigenerate.genomecenter.ucdavis.edu
#screen -S PCB
#module load R/4.0.1
#R

# Install packages ###
#BiocManager::install(c("scales", "openxlsx", "rlist", "tidyverse", "ggdendro", "cowplot", "annotatr", "rtracklayer", 
"bsseq", "dmrseq", "WGCNA", "sva", "rGREAT", "R.devices", "biomaRt","rlang",
"TxDb.Hsapiens.UCSC.hg38.knownGene", "org.Hs.eg.db"))

# Load Packages ####
#.libPaths("/share/lasallelab/programs/comethyl/R_4.0.1")
#setwd("/share/lasallelab/Jules/MARBLES_PCB/Females_Males/V4")
#AnnotationHub::setAnnotationHubOption("CACHE", value = "/share/lasallelab/programs/comethyl/R_4.0.1")
sapply(c("scales", "openxlsx", "rlist", "tidyverse", "ggdendro", "cowplot", "annotatr", "rtracklayer", "bsseq", "dmrseq",
         "WGCNA", "sva", "rGREAT", "R.devices", "biomaRt", "rlang","TxDb.Hsapiens.UCSC.hg38.knownGene", "org.Hs.eg.db"), require, character.only = TRUE)
...
getRegions <- function(bs, annotation = NULL, genome = c("hg38"),
...
getModules <- function(meth, power, regions, maxBlockSize = 40000, corType = c("pearson", "bicor"),
                       maxPOutliers = 0.1, deepSplit = 4, minModuleSize = 30, mergeCutHeight = 0.1, nThreads = 4, save = TRUE,
...
plotMEtraitCor <- function(MEtraitCor, moduleOrder = 1:length(unique(MEtraitCor$module)),
                           ...
                           legend.title.size = 16, colColorMargins = c(-0.7,4.0,1.2,9.2), save = TRUE,
  ...
  heatmap <- ggplot(data = MEtraitCor) +
    ...
    scale_alpha_manual(breaks = c("TRUE", "FALSE"), values = c("TRUE" = 1, "FALSE" = 0), guide = "none") +
    theme_bw(base_size = 24) +
...
#Load data 
colData <- read.xlsx("/Users/juliamouat/Library/Mobile Documents/com~apple~CloudDocs/LaSalle Lab/MARBLES PCB/Comethyl/V5/Sample_Info_V5.xlsx", rowNames=TRUE)
modules <- readRDS("/Users/juliamouat/Library/Mobile Documents/com~apple~CloudDocs/LaSalle Lab/MARBLES PCB/Comethyl/V3/Females_Males/Modules.rds")
...
MEtraitCor <- getMEtraitCor(MEs, colData = colData, corType = "bicor", adjMethod = "none", file = "ME_Trait_Correlation_Stats.txt")
...
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order,
               file = "ME_Trait_Correlation_Heatmap_2.pdf")
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order,
               file = "ME_Trait_Correlation_Heatmap_3.pdf")
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order,
               file = "ME_Trait_Correlation_Heatmap_4.pdf")
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order,
               file = "ME_Trait_Correlation_Heatmap_5.pdf")
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order,
               file = "ME_Trait_Correlation_Heatmap_6.pdf")
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order,
               file = "ME_Trait_Correlation_Heatmap_7.pdf")
plotMEtraitCor(MEtraitCor, moduleOrder = moduleDendro$order, traitOrder = traitDendro$order, sigOnly = TRUE, adj_p = 0.05, star.size = 11,
               star.nudge_y = -0.27, legend.position = c(1.14, 0.745), colColorMargins = c(-1,5.0,0.5,7.9),
               file = "Sig_ME_Trait_Correlation_Heatmap.pdf", width = 7, height = 3.5)

# Explore Significant ME-Trait Correlations ####
# Plot Module Eigennodes vs Traits - Non-continuous variables
plotMEtraitDot(MEs$lightgreen, trait = colData$Diagnosis, traitCode = c("TD" = 0, "non-TD" = 1, "ASD" = 2), 
               colors = c("TD" = "#3366CC", "non-TD" = "#258f1a", "ASD" = "#FF3366"), ylim = c(-0.2,0.2), xlab = "Diagnosis", 
               ylab = "Light Green Module Eigennode", file = "lightgreen_ME_Diagnosis_Dotplot.pdf")
plotMEtraitDot(MEs$salmon, trait = colData$Diagnosis, traitCode = c("TD" = 0, "non-TD" = 1, "ASD" = 2), 
               colors = c("TD" = "#3366CC", "non-TD" = "#258f1a", "ASD" = "#FF3366"), ylim = c(-0.2,0.2), xlab = "Diagnosis", 
               ylab = "Salmon Module Eigennode", file = "salmon_ME_Diagnosis_Dotplot.pdf")
plotMEtraitDot(MEs$blue, trait = colData$Diagnosis, traitCode = c("TD" = 0, "non-TD" = 1, "ASD" = 2), 
               colors = c("TD" = "#3366CC", "non-TD" = "#258f1a", "ASD" = "#FF3366"), ylim = c(-0.2,0.2), xlab = "Diagnosis", 
               ylab = "Blue Module Eigennode", file = "blue_ME_Diagnosis_Dotplot.pdf")
plotMEtraitDot(MEs$midnightblue, trait = colData$Diagnosis, traitCode = c("TD" = 0, "non-TD" = 1, "ASD" = 2), 
               colors = c("TD" = "#3366CC", "non-TD" = "#258f1a", "ASD" = "#FF3366"), ylim = c(-0.2,0.2), xlab = "Diagnosis", 
               ylab = "Midnight Blue Module Eigennode", file = "midnightblue_ME_Diagnosis_Dotplot.pdf")
plotMEtraitDot(MEs$midnightblue, trait = colData$Insertion, traitCode = c("No" = 0, "Yes" = 1), 
               colors = c("No" = "#3366CC", "Yes" = "#FF3366"), ylim = c(-0.2,0.2), xlab = "NHIP Insertion", 
               ylab = "Midnight Blue Module Eigennode", file = "midnightblue_ME_NHIP_Insertion_Dotplot.pdf")

# Plot Module Eigennodes vs Traits - Continuous variables
plotMEtraitScatter(MEs$lightgreen, trait = colData$ADOScs, ylim = c(-0.15,0.15), xlab = "ADOScs", 
                   ylab = "Light Green Module Eigennode", file = "lightgreen_ME_ADOScs_Scatterplot.pdf")
plotMEtraitScatter(MEs$blue, trait = colData$ADOScs, ylim = c(-0.15,0.15), xlab = "ADOScs", 
                   ylab = "Blue Module Eigennode", file = "blue_ME_ADOScs_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$PCB_177, ylim = c(-0.15,0.15), xlab = "PCB 177", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_PCB_177_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$PCB_146, ylim = c(-0.15,0.15), xlab = "PCB 146", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_PCB_146_Scatterplot.pdf")
plotMEtraitScatter(MEs$tan, trait = colData$MSLelcStandard, ylim = c(-0.15,0.15), xlab = "MSL elc Standard", 
                   ylab = "Tan Module Eigennode", file = "tan_ME_MSLelcStandard_Scatterplot.pdf")
plotMEtraitScatter(MEs$brown, trait = colData$MSLelcStandard, ylim = c(-0.15,0.15), xlab = "MSL elc Standard", 
                   ylab = "Brown Module Eigennode", file = "brown_ME_MSLelcStandard_Scatterplot.pdf")
plotMEtraitScatter(MEs$green, trait = colData$PCB_156_157, ylim = c(-0.15,0.15), xlab = "PCB 156_157 Standard", 
                   ylab = "Green Module Eigennode", file = "green_ME_MSLelcStandard_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$MSLelcStandard, ylim = c(-0.15,0.15), xlab = "MSL elc Standard", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_MSLelcStandard_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$PCB_187, ylim = c(-0.15,0.15), xlab = "PCB 187", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_PCB_187_Scatterplot.pdf")
plotMEtraitScatter(MEs$salmon, trait = colData$ADOScs, ylim = c(-0.15,0.15), xlab = "ADOScs", 
                   ylab = "Salmon Module Eigennode", file = "salmon_ME_ADOScs_Scatterplot.pdf")
plotMEtraitScatter(MEs$salmon, trait = colData$PCB_196, ylim = c(-0.15,0.15), xlab = "PCB 196", 
                   ylab = "Salmon Module Eigennode", file = "salmon_ME_PCB_196_Scatterplot.pdf")
plotMEtraitScatter(MEs$tan, trait = colData$PCB_169, ylim = c(-0.15,0.15), xlab = "PCB 169", 
                   ylab = "Tan Module Eigennode", file = "tan_ME_PCB_169_Scatterplot.pdf")
plotMEtraitScatter(MEs$royalblue, trait = colData$MSLelcStandard, ylim = c(-0.15,0.15), xlab = "MSL elc Standard", 
                   ylab = "Royal Blue Module Eigennode", file = "royalblue_ME_MSLelcStandard_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$PCB_170, ylim = c(-0.15,0.15), xlab = "PCB 170", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_PCB_170_Scatterplot.pdf")
plotMEtraitScatter(MEs$salmon, trait = colData$PCB_170, ylim = c(-0.15,0.15), xlab = "PCB 170", 
                   ylab = "Salmon Module Eigennode", file = "salmon_ME_PCB_170_Scatterplot.pdf")
plotMEtraitScatter(MEs$brown, trait = colData$PCB_177, ylim = c(-0.15,0.15), xlab = "PCB 177", 
                   ylab = "Brown Module Eigennode", file = "brown_ME_PCB_177_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$PCB_169, ylim = c(-0.15,0.15), xlab = "PCB 169", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_PCB_169_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$NDL_PCBs_Totals, ylim = c(-0.15,0.15), xlab = "Non-Dioxin-Like PCBs", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_NDL_PCBs_Scatterplot.pdf")
plotMEtraitScatter(MEs$brown, trait = colData$PCB_153_168, ylim = c(-0.15,0.15), xlab = "PCB 153_168", 
                   ylab = "Brown Module Eigennode", file = "brown_ME_PCB_153_168_Scatterplot.pdf")
plotMEtraitScatter(MEs$yellow, trait = colData$Total_PCBs, ylim = c(-0.15,0.15), xlab = "Total PCBs", 
                   ylab = "Yellow Module Eigennode", file = "yellow_ME_Total_PCBs_Scatterplot.pdf")
plotMEtraitScatter(MEs$midnightblue, trait = colData$ADOScs, ylim = c(-0.15,0.15), xlab = "ADOScs", 
                   ylab = "Midnight Blue Module Eigennode", file = "midnightblue_ME_ADOScs_Scatterplot.pdf")
